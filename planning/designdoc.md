...
TLDR see the ER diagram. But we need to build the following parts of the system:

- component (abstract class)
- component subclasses
- DAG API (`set_upstream`, etc) for components
- decorator for logging
- util functions to log final component run associated with `output_id`
- db for storage (including entities and relations in ER diagram)
- API to log to the db
- API to retrieve from the db (including `backtrace` call)
...
serve_component.set_owner('@shreyashankar')
...
Behind the scenes, our system would log the output id to a database with pointers to dynamic information generated by the `@register` decorator. Now, if there were a component that occurred before `serve` (`inference` for example), we could do the following:

```
# Create inference component
inference_component = create_or_retrieve_component('inference')
inference_component.set_description('some description')
inference_component.set_owner('@shreyashankar')

# Log artifact locations when this component is run
@register(component=inference_component, input_path=features_path, output_path=output_path, git_hash=...)
def inference(features_path, model_id):
    model = load_model(model_id)
    features = load_features(features_path)
    dump(model(features), output_path)
...
# Set serving component dependence on inference component
serve_component.set_upstream(inference_component)

# Log artifact locations when this component is run
@register(component=serve_component, input_path=path, output_path=None, git_hash=None)
def serve(path):
    ...
    # Log output id for backtrace to be seamless
    log(output_id, component=serve_component)
    return output
```

Note that the user just has to create (or retrieve) component objects and wrap their existing functions in a decorator that does all the logging. What could go wrong ???? (famous last words)

# Testing

We will need to test all the modules in the system. Some ideas of tests:

## Component and Python utils
...
- API tests
- creating new entities and relations
- uniqueness in components
- indexing

## Example on end to end pipeline

- notebook example
- "system" example where you have multiple files
- training & updating models (updating a version of a component)