...
class TestComponentRunHistory(unittest.TestCase):
    def setUp(self):
        ...
        self.historyLength = self.store.get_component_runs_count("mock_component")
        self.firstComponentRun = self.store.get_component_runs_by_index("mock_component", 0, 1)[0]
        self.lastComponentRun = self.store.get_component_runs_by_index("mock_component", self.historyLength - 1, self.historyLength)[0]
        self.secondAndThirdComponentRun = self.store.get_component_runs_by_index("mock_component", 1, 3)

    def testPstPstIndex(self):

        # case 1: (0, 0) return zero componentRun
        resCrList = self.store.get_component_runs_by_index("mock_component", 0, 0)
        self.assertEqual(len(resCrList), 0)

        # case 2: (0, 1) return the first componentRun
        resCrList = self.store.get_component_runs_by_index("mock_component", 0, 1)
        ...
        resCrList = self.store.get_component_runs_by_index("mock_component", self.historyLength - 1, self.historyLength)
        self.assertEqual(len(resCrList), 1)
        self.assertTrue(isEqualComponentRun(resCrList[0], self.lastComponentRun))

        # case 4: (1, 3) return the second and third componentRun given len(componentRun) >=3)
        resCrList = self.store.get_component_runs_by_index("mock_component", 1, 3)
        self.assertEqual(len(resCrList), 2)
        for idx, cr in enumerate(resCrList):
            if idx >= len(self.secondAndThirdComponentRun):
                break
            else:
                self.assertTrue(isEqualComponentRun(cr, self.secondAndThirdComponentRun[idx]))


    def testPstNgtIndex(self):

        # case 1: (0, -len(componentRun) + 1) return first componentRun)
        resCrList = self.store.get_component_runs_by_index("mock_component", 0, -self.historyLength + 1)
        self.assertEqual(len(resCrList), 1)
        self.assertTrue(isEqualComponentRun(resCrList[0], self.firstComponentRun))
        ...
        resCrList = self.store.get_component_runs_by_index("mock_component", 1, -self.historyLength + 3)
        self.assertEqual(len(resCrList), 2)
        for idx, cr in enumerate(resCrList):
             self.assertTrue(isEqualComponentRun(cr, self.secondAndThirdComponentRun[idx]))
    ...
    def testNgtPstIndex(self):
        ...
        resCrList = self.store.get_component_runs_by_index("mock_component", -self.historyLength, 1)
        self.assertEqual(len(resCrList), 1)
        self.assertTrue(isEqualComponentRun(resCrList[0], self.firstComponentRun))

        # case 2: (-1, len(componentRun)) return the last componentRun)
        resCrList = self.store.get_component_runs_by_index("mock_component", -1, self.historyLength)
        self.assertEqual(len(resCrList), 1)
        self.assertTrue(isEqualComponentRun(resCrList[0], self.lastComponentRun))

        # case 3: (-len(componentRun) + 1, 3) return second and third componentRun given len(componentRun) >=3)
        resCrList = self.store.get_component_runs_by_index("mock_component", -self.historyLength + 1, 3)
        self.assertEqual(len(resCrList), 2)
        for idx, cr in enumerate(resCrList):
             self.assertTrue(isEqualComponentRun(cr, self.secondAndThirdComponentRun[idx]))
    ...
    def testNgtNgtIndex(self):
        ...
        resCrList = self.store.get_component_runs_by_index("mock_component", -self.historyLength, -self.historyLength + 1)
        self.assertEqual(len(resCrList), 1)
        self.assertTrue(isEqualComponentRun(resCrList[0], self.firstComponentRun))
        ...
        resCrList = self.store.get_component_runs_by_index("mock_component", -self.historyLength + 1, -self.historyLength + 3)
        self.assertEqual(len(resCrList), 2)
        for idx, cr in enumerate(resCrList):
             self.assertTrue(isEqualComponentRun(cr, self.secondAndThirdComponentRun[idx]))
