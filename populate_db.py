...
for i in range(NUM_OUTER):
    ...
    @register(
        "training",
        inputs=[f"train_set_{i}.pq", f"test_set_{i}.pq"],
        outputs=[f"model_{i}.hdf5"],
    )
    def training():
    ...
    @register(
        "inference",
        inputs=[f"features_{i}.pq", f"model_{i}.hdf5"],
        outputs=[f"preds_{i}.pq"],
    )
    def inference():
    ...
    for j in range(NUM_INNER):
        ...
        @register(
            "serve",
            inputs=[f"preds_{i}.pq"],
            outputs=[f"serve_output_{idx}"],
            endpoint=True,
        )
        def serve():
